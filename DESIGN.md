...
- Ship silhouettes span each own-board hull once, using elongated class-specific
  profiles oriented with the vessel and scaled to its footprint; enemy class
  silhouettes remain hidden until that vessel is sunk, when the same class
...
- A newly sunk vessel fractures into two authored flat-profile halves over
  roughly 620ms: a hard 10px central gap opens, the halves separate with
  opposing 3px horizontal and 2px vertical translations, and the
  full-opacity halves remain at full scale as a readable wreck, transitioning
  from foam to burnt orange. The wreck profile uses a 66% cross-axis height
  plus a 6px trailing clip so the translated silhouette stays inside its cell
  run. Reduced motion skips directly to the final orange resting state. Fleet
  and legend sunk silhouettes use the same orange wreck color.
