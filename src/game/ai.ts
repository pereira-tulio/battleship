...
export const createAIState = (): AIState => ({ fired: new Set(), targets: [], hits: [] });

const unique = (cells: Coordinate[], fired: Set<string>) => cells.filter((cell) => !fired.has(keyOf(cell)));
const huntCandidates = (board: Board, state: AIState) => {
  const remaining = FLEET.filter((spec) => !board.ships.find((ship) => ship.name === spec.name && ship.hits.length >= ship.size));
  const parity = Math.max(remaining.reduce((max, ship) => Math.max(max, ship.size), 1), 1) % 2;
  const cells = Array.from({ length: BOARD_SIZE * BOARD_SIZE }, (_, i) => ({ row: Math.floor(i / BOARD_SIZE), col: i % BOARD_SIZE }))
    .filter((cell) => !state.fired.has(keyOf(cell)) && (cell.row + cell.col) % 2 === parity);
  return cells.length ? cells : Array.from({ length: BOARD_SIZE * BOARD_SIZE }, (_, i) => ({ row: Math.floor(i / BOARD_SIZE), col: i % BOARD_SIZE }))
...
export const aiFire = (board: Board, state: AIState, rng: RNG): { board: Board; state: AIState; coordinate: Coordinate; result: 'hit' | 'miss' | 'sunk' } => {
  ...
  const next: AIState = { fired: new Set(state.fired).add(keyOf(coordinate)), targets: state.targets, hits: state.hits };
  if (result.result === 'hit') {
    next.hits = [...state.hits, coordinate];
    next.targets = [...unique([...state.targets, ...neighbors(coordinate)], next.fired)];
    ...
    const sunkShip = board.ships.find((ship) => shipCells(ship).some((cell) => keyOf(cell) === keyOf(coordinate)));
    if (sunkShip) {
...
function shipCells(ship: Board['ships'][number]) { return Array.from({ length: ship.size }, (_, i) => ({ row: ship.start.row + (ship.orientation === 'vertical' ? i : 0), col: ship.start.col + (ship.orientation === 'horizontal' ? i : 0) })); }
export { allShipsSunk };
